...
import {
    ...
    User, Save, Coffee, CreditCard, Camera, Upload, Briefcase, Smile
} from 'lucide-react';

moment.locale('pt-br');

// MODAL DE CONFIRMAÇÃO (Card Feliz)
const ConfirmationModal = ({ onClose }) => (
    <div className="modal-overlay">
        <div className="modal-content">
            ...
            <h2 style={{ fontSize: '1.2rem', marginBottom: 8 }}>Agendado!</h2>
            <p style={{ color: '#6b7280', fontSize: '0.95rem', marginBottom: 20 }}>Seu horário foi reservado com sucesso.</p>
...
const Dashboard = ({ usuario, empresa }) => {
    ...
    const [loadingData, setLoadingData] = useState(true); // Carregamento de dados
    ...
    const carregarTudo = useCallback(async () => {
        // Não ativa setLoadingData se for apenas um refresh rápido, só na primeira vez ou refresh forçado
        try {
            // Promise.allSettled para não quebrar se uma rota falhar
            ...
            if (resS.status === 'fulfilled') setServicos(resS.value.data);
            if (resA.status === 'fulfilled') setAgenda(resA.value.data);

            // Filtra Equipe: Remove 'admin_geral' e remove o próprio usuário se ele for dono mas não atende (opcional)
            if (resE.status === 'fulfilled') {
                const profissionaisValidos = resE.value.data.filter(p => p.role !== 'admin_geral');
                setEquipe(profissionaisValidos);
            }
    ...
    const handleService = async (e) => {
        ...
        try {
            ...
            setSvcForm({ nome: '', descricao: '', preco: '', duracao_minutos: '' }); setEditId(null); carregarTudo();
            alert("Salvo!");
        } catch (e) { alert("Erro"); }
    };
    ...
    const addMembro = async (e) => {
        e.preventDefault();
        try { await api.post('/equipe', novoMembro); setNovoMembro({ nome: '', email: '', senha: '', role: 'profissional' }); carregarTudo(); alert("Adicionado!"); } catch (e) { alert(e.response?.data?.erro || "Erro"); }
    };
        ...
        <div className="layout">
            ...
            <aside className="sidebar">
                ...
                <nav style={{ flex: 1, display: 'flex', flexDirection: window.innerWidth <= 768 ? 'row' : 'column', gap: 6 }}>
                    ...
                    {/* MENUS DO DONO (GARANTIDO QUE FUNCIONA) */}
                    {usuario.role === 'dono' && (
                        <>
            ...
            <main className="main-content">
                ...
                <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 30 }} className="page-header">
                    <div>
                        ...
                        <p style={{ margin: 0, color: 'var(--text-muted)' }}>Gerencie com facilidade.</p>
                    </div>
                    {/* FOTO DO USUÁRIO NO TOPO */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        {usuario.foto_url ? (
                            <img src={usuario.foto_url} style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--primary)' }} />
                        ) : (
                ...
                {/* SPINNER DE LOADING (Substitui tela branca) */}
                {loadingData ? (
                    <div style={{ padding: 50, textAlign: 'center' }}>
                        <div className="spinner"></div>
                        <p style={{ marginTop: 10, color: 'var(--text-muted)', fontSize: '0.9rem' }}>Carregando...</p>
                    </div>
                ) : (
                    <>
                        {/* --- ABA AGENDA --- */}
                            ...
                            <div className="grid-2-col">
                                <div className="card">
                                    ...
                                    <form onSubmit={handleAgendar} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                        {usuario.role === 'dono' && (
                                            <div><label className="label">Cliente Avulso</label><input className="input" placeholder="Nome" value={novoAgendamento.nome} onChange={e => setNovoAgendamento({ ...novoAgendamento, nome: e.target.value })} /></div>
                                        )}
                                        ...
                                        ) : novoAgendamento.data && <div style={{ textAlign: 'center', color: '#999', padding: 10, background: '#f3f4f6', borderRadius: 8 }}>Sem horários livres.</div>}
                                        <button type="submit" className="btn btn-cta" disabled={!novoAgendamento.hora}>Confirmar Agendamento</button>
                                    </form>
                                </div>

                                <div className="card">
                                    <h3>{usuario.role === 'dono' ? 'Todos Agendamentos' : 'Meus Horários'}</h3>
                                    {agenda.length === 0 ? <p style={{ color: '#999' }}>Nenhum agendamento.</p> : (
                                        <div>
                                            {agenda.map(ag => (
                                                <div key={ag.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid #eee' }}>
                                                    <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                                                        <div style={{ background: '#eff6ff', padding: '8px 12px', borderRadius: 8, textAlign: 'center', color: 'var(--primary)' }}>
                                                        ...
                                                        <div>
                                                            ...
                                                            <div style={{ fontSize: '0.85rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                        ...
                        {aba === 'perfil' && (
                            <div className="card" style={{ maxWidth: 600, margin: '0 auto' }}>
                                <div className="avatar-area" onClick={() => fileRef.current.click()}>
                                    {perfil.foto_url ? <img src={perfil.foto_url} className="avatar-img" /> : <div className="avatar-img" style={{ background: '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30 }}>{perfil.nome.charAt(0)}</div>}
                                ...
                                <form onSubmit={salvarPerfil} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                    ...
                                    <div><label className="label">Nova Senha</label><input className="input" type="password" value={perfil.senha} onChange={e => setPerfil({ ...perfil, senha: e.target.value })} placeholder="Opcional" /></div>

                                    {usuario.role === 'dono' && (
                                        <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid #eee' }}>
                                            <h3 style={{ marginBottom: 15 }}>Empresa</h3>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 15, marginBottom: 15 }}>
                                                <div style={{ width: 60, height: 60, borderRadius: 10, background: '#f3f4f6', overflow: 'hidden', cursor: 'pointer' }} onClick={() => logoRef.current.click()}>
                                                    {empresaConfig.logo_url ? <img src={empresaConfig.logo_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Upload size={20} /></div>}
                                                </div>
                            ...
                            <div className="grid-2-col">
                                <div className="card" style={{ height: 'fit-content' }}>
                                    ...
                                    <form onSubmit={handleService} style={{ display: 'grid', gap: 16 }}>
                                        <div><label className="label">Nome</label><input className="input" value={svcForm.nome} onChange={e => setSvcForm({ ...svcForm, nome: e.target.value })} required /></div>
                                        ...
                                        <div style={{ display: 'flex', gap: 10 }}>
                                            <div style={{ flex: 1 }}><label className="label">Preço</label><input className="input" type="number" value={svcForm.preco} onChange={e => setSvcForm({ ...svcForm, preco: e.target.value })} required /></div>
                                ...
                                <div className="card">
                                    <h3>Lista</h3>
                                    {servicos.map(s => (
                                        <div key={s.id} style={{ padding: '12px 0', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between' }}>
                                            <div><div style={{ fontWeight: 600 }}>{s.nome}</div><div style={{ fontSize: '0.85rem', color: '#666' }}>R$ {s.preco} • {s.duracao_minutos}m</div></div>
                                            <div style={{ display: 'flex', gap: 5 }}>
                                                <button onClick={() => { setEditId(s.id); setSvcForm(s) }} className="btn" style={{ padding: 5 }}><Edit2 size={16} /></button>
                                                <button onClick={() => deleteService(s.id)} className="btn" style={{ padding: 5, color: 'red' }}><Trash2 size={16} /></button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        {aba === 'equipe' && (
                            <div className="grid-2-col">
                                ...
                                <div className="card">
                                    <h3>Membros</h3>
                                    {equipe.map(m => (
                                        <div key={m.id} style={{ padding: '12px 0', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                <div style={{ width: 32, height: 32, background: '#f3f4f6', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>{m.nome.charAt(0)}</div>
                                                <div><div style={{ fontWeight: 600 }}>{m.nome}</div><div style={{ fontSize: '0.8rem', color: '#666' }}>{m.role}</div></div>
                                            </div>
                                            {m.id !== usuario.id && <button onClick={() => removeMembro(m.id)} className="btn" style={{ padding: 5, color: 'red' }}><Trash2 size={16} /></button>}
                                        </div>
                                    ))}
                                </div>
                            </div>
